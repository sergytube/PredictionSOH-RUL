%% PREDIZIONE SOH A PARTIRE DA QUESTE FEATURES 'HI1','HI2','HI3'--- MSE Error: 1.6379
...
X_train = [data_B5.features; data_B6.features;data_B7.features];
y_train = [data_B5.B5.SOH; data_B6.B6.SOH;data_B7.B7.SOH];


X_test = data_B18.features;
X_test_MSE = data_B18.features(25:end,:);
y_test = data_B18.B18.SOH(25:end,:); 

y_test_tot = data_B18.B18.SOH(1:134,:);
y_test_tot1 = data_B18.B18.SOH(1:24,:);
y_test_tot2 = data_B18.B18.SOH(51:100,:);
...
numFolds = 10; % Numero di fold per la cross-validation
...
cycles = data_B18.B18.Num_cycle;  % Assumendo che ci sia una colonna Num_cycle
...
first_under_80_real = find(data_B18.B18.SOH < 80, 1);

if ~isempty(first_under_80_real) && first_under_80_real <= length(data_B18.B18.SOH) - 5
    if all(data_B18.B18.SOH(first_under_80_real+1:first_under_80_real+5) < 80)
        first_under_80_consecutive_real = first_under_80_real;
    else
        first_under_80_consecutive_real = 62; % Nessun gruppo di 5 valori consecutivi sotto l'80%
    end
else
...
%% Plot dei risultati
figure;
plot(25:134, y_test, 're-', 'LineWidth', 1.5); % Dati reali
hold on;
plot(25:134, predictions_MSE, 'bl-', 'LineWidth', 1.5); % Predizioni
hold on;
...
save('prediction_SOH_B18', 'predictions');
